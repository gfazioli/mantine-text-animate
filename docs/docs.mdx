...
## TextAnimate.Gradient component

The `TextAnimate.Gradient` component allows you to display text with an animated gradient background.

<Demo data={demos.gradientConfigurator} />

## TextAnimate.Highlight component
...
## TextAnimate.Scramble component

The `TextAnimate.Scramble` component creates a "hacker/decryption" effect where each character rapidly cycles through random characters before settling on the target text. Each character starts scrambling at a staggered time and settles after `scrambleDuration` milliseconds.

<Demo data={demos.scrambleConfigurator} />

## useScramble() hook

The `TextAnimate.Scramble` is built by using the `useScramble` hook.

<Demo data={demos.scrambleHook} />

## TextAnimate.SplitFlap component
...
## TextAnimate.Morphing component
